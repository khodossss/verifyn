...
from verifyn.agent.agent import _extract_result
...
class TestDirectParse:
    def test_valid_json_block_parsed_without_llm(self):
        ...
        with patch("verifyn.agent.agent._build_llm") as mock_llm:
...
class TestRepairFallback:
    ...
    def test_invalid_json_block_triggers_repair(self):
        ...
        with patch("verifyn.agent.agent._build_llm", return_value=self._mock_llm_response(valid_json)):
            result = _extract_result(narrative)
        assert result.verdict == Verdict.REAL
    ...
    def test_repair_failure_raises_runtime_error(self):
        ...
        with patch("verifyn.agent.agent._build_llm", return_value=self._mock_llm_response("not json at all")):
    ...
    def test_repair_call_receives_broken_json(self):
        ...
        with patch("verifyn.agent.agent._build_llm", return_value=llm):
            _extract_result(narrative)
