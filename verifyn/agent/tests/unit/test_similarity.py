...
from verifyn.agent.db import Base, _get_engine
from verifyn.agent.tools.similarity import _format_previous_result, search_similar_queries
...
def _disable_claim_prefilter():
    """Block the classifier so similarity tests do not load DeBERTa."""
...
class TestSearchSimilarQueriesTool:
    ...
    @patch("verifyn.agent.db.compute_embedding")
    @patch("verifyn.agent.db.find_similar_queries")
...
class TestAgentModeInjection:
    ...
    @patch("verifyn.agent.db.compute_embedding", return_value=[0.1] * 8)
    def test_fast_mode_sets_tool_mode(self, mock_embed, mock_build_llm, mock_create_agent):
        ...
        from verifyn.agent.tools.similarity import search_similar_queries as sim_tool
