...
from verifyn.agent.agent import (
    _build_user_message,
    _extract_narrative,
    _sanitize_json,
    _try_parse,
...
def _disable_claim_prefilter():
    """Pin the pre-filter to "always pass" so existing tests do not load DeBERTa.

    Tests that exercise the pre-filter directly override this with their own
    `patch("verifyn.agent.agent._claim_prefilter", ...)`.
    """
...
class TestAnalyzeNews:
    @patch("verifyn.agent.agent.create_react_agent")
    @patch("verifyn.agent.agent._build_llm")
...
class TestClaimPrefilter:
    """Pre-filter short-circuits the agent when the classifier rejects input."""

    def _make_no_claims_result(self) -> FactCheckResult:
        from verifyn.agent.models import ConfidenceLevel, ManipulationType
        ...
        return FactCheckResult(
            ...
            reasoning="Pre-filter rejected: classifier score 0.03 below threshold.",
            summary="No verifiable claims (classifier score 0.03).",
        )

    @patch("verifyn.agent.agent.create_react_agent")
    @patch("verifyn.agent.agent._build_llm")
    def test_short_circuits_analyze_news_without_calling_llm(self, mock_build_llm, mock_create_agent):
        no_claims = self._make_no_claims_result()
        with patch("verifyn.agent.agent._claim_prefilter", return_value=no_claims):
            result = analyze_news("not a claim, just chatter")
    ...
    @patch("verifyn.agent.agent.create_react_agent")
    @patch("verifyn.agent.agent._build_llm")
    def test_passes_through_when_prefilter_returns_none(self, mock_build_llm, mock_create_agent):
        mock_create_agent.return_value = _mock_react_agent(_agent_final_message())
        mock_build_llm.return_value = MagicMock()
    ...
    @patch("verifyn.agent.agent.create_react_agent")
    @patch("verifyn.agent.agent._build_llm")
    def test_short_circuits_analyze_news_stream(self, mock_build_llm, mock_create_agent):
        no_claims = self._make_no_claims_result()
        with patch("verifyn.agent.agent._claim_prefilter", return_value=no_claims):
            events = list(analyze_news_stream("hello there"))

        types = [e["type"] for e in events]
        assert "result" in types
        assert "thinking" in types  # we yield a single thinking event explaining the short-circuit
        assert "tool_call" not in types
        result_event = next(e for e in events if e["type"] == "result")
        assert result_event["data"]["verdict"] == "NO_CLAIMS"
        mock_build_llm.assert_not_called()
        mock_create_agent.assert_not_called()
