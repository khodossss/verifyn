...
from verifyn.agent.tools.search import _format_results, check_if_old_news, search_fact_checkers, web_search
...
class TestWebSearch:
    @patch("verifyn.agent.tools.search._tavily_search", return_value=SAMPLE_RESULTS)
    ...
    @patch("verifyn.agent.tools.search._ddg_search", return_value=SAMPLE_RESULTS)
    @patch("verifyn.agent.tools.search._tavily_search", side_effect=Exception("no key"))
    def test_ddg_fallback_on_tavily_failure(self, mock_tavily, mock_ddg):
        result = web_search.invoke({"query": "test query"})
        mock_ddg.assert_called_once()
        assert "BBC News" in result

    @patch("verifyn.agent.tools.search._ddg_search", side_effect=Exception("ddg down"))
    @patch("verifyn.agent.tools.search._tavily_search", side_effect=Exception("tavily down"))
    def test_both_fail_returns_error_string(self, mock_tavily, mock_ddg):
        result = web_search.invoke({"query": "test"})
        assert "Search failed" in result

    @patch("verifyn.agent.tools.search._tavily_search", return_value=[])
...
class TestSearchFactCheckers:
    ...
    @patch("verifyn.agent.tools.search._ddg_search", return_value=SAMPLE_RESULTS)
    @patch("verifyn.agent.tools.search._tavily_search", side_effect=Exception("fail"))
...
class TestCheckIfOldNews:
    ...
    @patch("verifyn.agent.tools.search._tavily_search", side_effect=Exception("fail"))
    @patch("verifyn.agent.tools.search._ddg_search", side_effect=Exception("fail"))
