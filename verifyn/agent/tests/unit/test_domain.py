...
from verifyn.agent.tools.domain import _extract_domain, _web_reputation_search, check_domain_reputation
...
class TestWebReputationSearch:
    def test_returns_snippets_on_success(self):
        ...
        with patch("verifyn.agent.tools.search._search_with_fallback", return_value=fake_results):
    ...
    def test_falls_back_to_broader_query(self):
        ...
        with patch("verifyn.agent.tools.search._search_with_fallback", side_effect=[[], broad_results]):
    ...
    def test_returns_empty_string_when_all_fail(self):
        with patch("verifyn.agent.tools.search._search_with_fallback", return_value="Search failed: error"):
            result = _web_reputation_search("unknown.com")
        assert result == ""

    def test_returns_empty_on_no_results(self):
        with patch("verifyn.agent.tools.search._search_with_fallback", return_value=[]):
    ...
    def test_limits_output_to_3_snippets(self):
        ...
        with patch("verifyn.agent.tools.search._search_with_fallback", return_value=many_results):
            result = _web_reputation_search("example.com")
...
class TestCheckDomainReputation:
    ...
    def test_unknown_domain_triggers_web_search(self):
        with patch(
            "verifyn.agent.tools.domain._web_reputation_search", return_value="Rated LOW credibility"
        ) as mock_web:
    ...
    def test_unknown_domain_no_web_results(self):
        with patch("verifyn.agent.tools.domain._web_reputation_search", return_value=""):
            result = self._invoke("randomsite123.com")
    ...
    def test_db_confident_skips_web_search(self):
        ...
        with (
            patch("verifyn.agent.db.get_domain_credibility", return_value=db_info),
            patch("verifyn.agent.db.CREDIBILITY_THRESHOLD", 50),
            patch("verifyn.agent.tools.domain._web_reputation_search") as mock_web,
        ):
    ...
    def test_db_below_threshold_still_runs_web_search(self):
        ...
        with (
            patch("verifyn.agent.db.get_domain_credibility", return_value=db_info),
            patch("verifyn.agent.db.CREDIBILITY_THRESHOLD", 50),
            patch("verifyn.agent.tools.domain._web_reputation_search", return_value="Some info") as mock_web,
        ):
