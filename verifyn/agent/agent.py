...
from .models import ConfidenceLevel, FactCheckResult, ManipulationType, Verdict
...
def _no_claims_result(score: float) -> FactCheckResult:
    """Build a short-circuit FactCheckResult when the pre-filter rejects the input."""
    ...
    return FactCheckResult(
        ...
        sources_checked=[],
        reasoning=(
            f"Claim-detection pre-filter classified the input as non-claim "
            f"(claim score = {score:.3f} < threshold {CLAIM_SCORE_THRESHOLD:.3f}). "
            "The text does not contain a verifiable factual assertion, so the "
            "ReAct research loop was skipped."
        ),
        summary=f"Input contains no verifiable factual claims (classifier score {score:.2f}).",
    )


def _claim_prefilter(news_text: str) -> FactCheckResult | None:
    """Run the cheap claim classifier before the agent.

    Returns a NO_CLAIMS FactCheckResult when the input is rejected by the
    pre-filter. Returns None when the agent should proceed normally — either
    because the score is above the threshold or because the classifier is
    unavailable (no model installed, torch missing, etc.).
    """
    if not CLAIM_DETECTOR_ENABLED:
        return None
    try:
        from verifyn.claim_detector import score_claim
    except ImportError as exc:
        logger.debug("Claim detector module unavailable: %s", exc)
        return None

    try:
        score = score_claim(news_text)
    except Exception as exc:
        logger.warning("Claim detector inference failed: %s", exc)
        return None

    if score is None:
        # Soft fallback: classifier present but cannot run (e.g. weights missing)
        logger.debug("Claim detector returned None, skipping pre-filter")
        return None

    if score < CLAIM_SCORE_THRESHOLD:
        logger.info(
            "Claim detector pre-filter rejected input: score=%.4f < threshold=%.4f",
            score,
            CLAIM_SCORE_THRESHOLD,
        )
        return _no_claims_result(score)

    logger.info("Claim detector pre-filter passed: score=%.4f >= threshold=%.4f", score, CLAIM_SCORE_THRESHOLD)
    return None
...
def analyze_news(news_text: str, *, verbose: bool = False, reasoning_effort: str | None = None) -> FactCheckResult:
    ...
    # Mode must be set even when the pre-filter short-circuits, so any
    # subsequent call to the similarity tool sees the correct value.
    from .tools.similarity import search_similar_queries as _sim_tool

    _sim_tool._current_mode = mode

    # Cheap pre-filter: classifier short-circuit before any LLM / embedding / DB call
    prefilter_result = _claim_prefilter(news_text)
    if prefilter_result is not None:
        if not eval_mode:
            try:
                from .db import save_query

                save_query(news_text, mode, prefilter_result, reputation_updated=0, embedding=None)
            except Exception as exc:
                logger.warning("Failed to persist pre-filter result: %s", exc)
        elapsed = _time.perf_counter() - t0
        logger.info("analyze_news short-circuited by claim detector in %.2fs", elapsed)
...
def analyze_news_stream(news_text: str, reasoning_effort: str | None = None) -> Generator[dict, None, None]:
    ...
    # Cheap pre-filter: skip the agent entirely for non-claim inputs
    prefilter_result = _claim_prefilter(news_text)
    if prefilter_result is not None:
        yield {"type": "thinking", "text": "Claim detector classified the input as non-claim. Skipping research."}
        if not eval_mode:
            try:
                from .db import save_query
                ...
                logger.warning("Failed to persist pre-filter result: %s", exc)
        yield {"type": "result", "data": prefilter_result.model_dump(mode="json")}
        return
